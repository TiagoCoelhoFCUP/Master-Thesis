...
temp.columns=['dominant_topic_0','dominant_topic_1','dominant_topic_2','dominant_topic_3','dominant_topic_4']
df = pd.concat([df,temp], axis=1)

temp = pd.get_dummies(df.topic)
temp.columns=['diversity_1','diversity_2','diversity_3','diversity_4','diversity_5']
df = pd.concat([df,temp], axis=1)
...
x = df[['is_quote_status','is_retweet_status','topic','topic_perc','topic_0','topic_1','topic_2','topic_3','topic_4','polarity','weekday','hour','month','user.followers_count']] #independent columns
...
featureScores.drop(featureScores.loc[featureScores['Score'] < 0.038].index, inplace=True)
...
x = df[['topic_0','topic_1','topic_2','topic_3','topic_4','topic','topic_perc','is_retweet_status','polarity','user.followers_count']].to_numpy()
...
ridge = Ridge(alpha=0.0, copy_X=True, fit_intercept=True, normalize=False, solver='auto')

"""Lasso Regression"""
from sklearn.linear_model import Lasso
lasso = Lasso(alpha=0.0, copy_X=True, fit_intercept=True, normalize=False, selection='cyclic', warm_start=True)

"""Decision Tree"""
from sklearn.tree import DecisionTreeRegressor
dt = DecisionTreeRegressor(ccp_alpha=0.1125, criterion='mse', max_depth=50, max_features='auto', min_impurity_decrease=0.1, min_samples_split=10, splitter='random')

"""Decision Tree Bagging"""
from sklearn.ensemble import BaggingRegressor
bagging = BaggingRegressor(bootstrap=True, bootstrap_features=True, max_features=4, max_samples=4994, n_estimators=3, warm_start=False)

"""Adaptive Boosting"""
from sklearn.ensemble import AdaBoostRegressor
boosting = AdaBoostRegressor(learning_rate=0.04040404040404041, loss='square', n_estimators=31)

"""KNN"""
from sklearn.neighbors import KNeighborsRegressor

x = np.nan_to_num(x)
knn = KNeighborsRegressor(algorithm='ball_tree', leaf_size=1, metric='manhattan', n_neighbors=67, weights='distance')

"""SVM"""
from sklearn.svm import SVR
svm = SVR(C=10, degree=1, kernel='rbf', cache_size=7000)

"""Random Forest"""
from sklearn.ensemble import RandomForestRegressor
rf = RandomForestRegressor(criterion='mae', max_depth=10, max_features='log2', min_samples_split=6, n_estimators=75)

"""MLP"""
from sklearn.neural_network import MLPRegressor
mlp = MLPRegressor(activation='tanh', early_stopping=False, hidden_layer_sizes=(10, 20, 40), learning_rate='constant')
